const Employee = require('../models/employeeModel'); // Fixed the 'models' path!
...
exports.getAllEmployees = async (req, res) => {
  try {
    const query = "SELECT id, firstName, lastName, email, employeeId, designation, staffType, institute_code FROM employees ORDER BY id DESC";
...
exports.deleteEmployee = async (req, res) => {
  try {
    ...
    if (err.code === 'ER_ROW_IS_REFERENCED_2') {
        return res.status(409).json({ success: false, message: "Cannot delete this employee. They are assigned to a department." });
    }
    res.status(500).json({ success: false, message: "Database Error" });
  }
};